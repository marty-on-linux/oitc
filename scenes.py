...
class MenuScene(Scene):
    ...
    def draw(self, display):
        ...
        for i, item in enumerate(self.menu_items):
            ...
            it_rect = it_surf.get_rect(center=(self.game.window_res[0]//2, 250 + i*40))
            display.blit(it_surf, it_rect)
...
class SettingsScene(Scene):
    ...
    def draw(self, display):
        ...
        title_rect = title.get_rect(center=(self.game.window_res[0]//2, 60))
        ...
        for i, key in enumerate(self.settings_items):
            ...
            rect = surf.get_rect(center=(self.game.window_res[0]//2, 150 + i*50))
...
class UpgradesScene(Scene):
    ...
    def draw(self, display):
        ...
        title_rect = title.get_rect(center=(self.game.window_res[0]//2, 40))
        display.blit(title, title_rect)

        # Draw tabs
        for i, tab_name in enumerate(self.tabs):
            color = self.game.biolum if i == self.current_tab_index else self.game.foam
            surf = self.game.font.render(tab_name, True, color)
            rect = surf.get_rect(center=(self.game.window_res[0]//2 + (i * 150) - 75, 100))
            display.blit(surf, rect)
        ...
        for i, up in enumerate(current_upgrades):
            ...
            surf = self.game.font.render(label, True, color)
            rect = surf.get_rect(center=(self.game.window_res[0]//2, 200 + i*50))
            display.blit(surf, rect)

        cur = self.game.font.render(f"Coins: {self.game.score}", True, self.game.biolum)
        cur_rect = cur.get_rect(topright=(self.game.window_res[0] - 20, 20))
        display.blit(cur, cur_rect)
