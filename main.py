...
from entities import Player, Bullet, Enemy, Upgrade
...
class Game:
    def __init__(self):
        ...
        self.upgrades = [
            Upgrade("Increase Max Ammo", "max_ammo", 5, 10, 1.5),
            Upgrade("Increase Player Speed", "player_speed", 1, 15, 1.8),
            Upgrade("Increase Bullet Speed", "bullet_speed", 1, 12, 1.6),
            Upgrade("Increase Shield Duration", "shield_duration", 0.5, 20, 1.5),
            Upgrade("Decrease Shield Cooldown", "shield_cooldown", 0.5, 20, 1.5),
        ]
    ...
    def create_enemy_sprite(self, path, size=32):
        surf = pygame.Surface((size, size), pygame.SRCALPHA)
        pygame.draw.rect(surf, (0, 0, 0, 0), surf.get_rect())
        pygame.draw.rect(surf, (255, 0, 0), (0, 0, size, size))
        pygame.image.save(surf, path)
    ...
    def generate_assets(self):
        ...
        enemy_sprite_path = os.path.join(self.assets_dir, 'enemy.png')
        try:
            ...
            if not os.path.isfile(enemy_sprite_path):
                self.create_enemy_sprite(enemy_sprite_path)
